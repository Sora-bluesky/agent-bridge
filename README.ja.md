...
|---|---|
| `stored` | DB へ届いていて、まだ誰も取っていない |
| `presented` | あるセッションが本文を渡され、15分以内に ack する区間 |
| `acked` | 渡されたプロセスが `bridge_ack` を呼んだ |

4つ目に `claimed` がありますが、**呼び出し側からは観測できません**。fetch は1回の呼び出しの中で claim と presented を続けて行うので、2分の lease はその隙間で落ちた場合しか覆いません。本文が出たあとは15分のほうが効きます。

**人が見たかどうかは、この3つのどれからも出ません。** 答える義務が生じるわけでもありません。依頼も通知も同じ1行で、ack した時点でどちらも DB 上は終わりです。
